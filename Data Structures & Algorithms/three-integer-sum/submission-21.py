...
# Optimal Solution
    # Goal : O(N^3)-> O(N^2), Two pointer technique : O(N)->O(1)
    ...
    # Tricks:
        ...
        # If the question state "Should not contain any duplicate" : 
            # if "condition" and nums[i-1] == nums[i]: continue -> next iteration  for loop
            # while "condition" and nums[l-1] == nums[l], l+=1 -> next iteration for while loop 
