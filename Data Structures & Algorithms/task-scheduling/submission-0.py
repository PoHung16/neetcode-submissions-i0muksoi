class Solution:
    def leastInterval(self, tasks: list[str], n: int) -> int:
        # Step 1: Frequency count arrat & Sort to find the leader
        count = [0] * 26
        for task in tasks:
            count[ord(task) - ord('A')] += 1
        count.sort()
        max_f = count[25]
        # Step 2: Calculate Idle Slots & Subtract  
        #先算出「老大」之間總共需要多少個空位（idle），然後遍歷其他任務，把這些空位填滿。
        #最後剩下來填不滿的 idle 加上原本的任務總數就是答案
