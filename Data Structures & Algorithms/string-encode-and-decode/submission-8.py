"""
 OOD: Yes- As you can see in Example
    ...
    #  B.Decide the data strucure
        #Encode: Implement it with empty list, traverse the string and write its size and “#”, Then join at the end
        #Decode: Implement it with while loop + 2 pointer to trace the index of the string and  trace delimeter position to get the length to append to final string
...
class Solution:
    ...
    def decode(self, string: str) -> List[str]:
        i = 0 # trace the index of the string
        res = []
        while i < len(string):
            j = i # trace the position of delimeter
            while string[j] != "#":
                j+=1
