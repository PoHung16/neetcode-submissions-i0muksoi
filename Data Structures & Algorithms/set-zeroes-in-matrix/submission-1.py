class Solution:
    def setZeroes(self, matrix: List[List[int]]) -> None:
        ...
        rows = len(matrix)
        cols = len(matrix[0])
        #Step 1 : Scan & Flag：掃描整個矩陣除了第一列當備忘錄。
        #為了不讓 (0,0) 身兼二職，我們請一個「校外人士」來幫忙：
        #matrix[0][0]：只負責 Column 0 (直的) 的開關。
        #rowZero 變數：只負責 Row 0 (橫的) 的開關。
        #如果它是 Row 0 的人，我們不准它動 matrix[0][0]。，避免誤殺
        for r in range(rows):
            for c in range(cols):
                if matrix[r][c] == 0:
        ...
        for r in range(1,rows):
            for c in range(1,cols):
                if matrix[0][c] == 0 or matrix[r][0] == 0:
                    matrix[r][c] = 0
        #Step 3: Handle MEMO - First ROW & TOP-LEFT corner：最後根據開頭記下的變數，處理第一行和第一列自己的命運
        if matrix[0][0] == 0:
            for r in range(rows):
                matrix[r][0] = 0
