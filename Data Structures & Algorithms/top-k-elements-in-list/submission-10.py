...
"""
# Brute Force: 
    # Array - Use hashMap to count frequency, and sort the hashmap's value, and get the top k ->O(N+NlogN)
class Solution:
    def topKFrequent(self,nums:List[int],k:int)->List[int]:
        hashMap = {}
        for i in range(len(nums)):
            hashMap[nums[i]] = hashMap.get(nums[i],0)+1
        res = sorted(hashMap.keys(),key=lambda x: hashMap[x],reverse=True)
        return res[0:k]
...
# Optimal Soltion:
    ...
    # Tricks
        # 1. If you want to traverse a map, map.items() 
