...
# Tricks
    ...
    # If its coin dp problem, dp[i] is used to store how many ways of coins to get i value 
...
class Solution:
    def coinChange(self, coins:List[int], amount:int) ->int:
        ...
        for i in range(1,amount+1): # 1 to amount
            for coin in coins:
                if i-coin>=0:
                    dp[i] = min(dp[i], 1+dp[i-coin]) # 狀態轉移：不選這個硬幣(維持 dp[i]) vs 選這個硬幣(這 1 枚硬幣1way  再加上 dp[i - coin]的方法數)
