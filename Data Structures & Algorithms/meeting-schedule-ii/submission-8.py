...
"""
# Brute Force: O(N^2)
    # Keyword: "Intervals Problem" -> Linear Scan Intervals (Sort + Single/Nested Loop)
        # Edge case
    # Approach
...
# Optimal Solution
    # Goal: O(N^2)-> O(N)
    # Keyword: "Intervals Problem" -> Linear Scan Intervals (Sort + Single/Nested Loop)-># Edge case
    # Keyword: "Return the Minimum number" -> Greedy 
...
class Solution:
    def minMeetingRooms(self, intervals: List[Tuple[int]]) -> int:
        ...
        min_heap = []
        # 3. Single Loop to traverse the interval,Check room's for EARLIEST available room 
        for meeting in intervals:
