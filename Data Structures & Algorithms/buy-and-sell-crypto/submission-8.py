...
# Image :  DP is remembering past results to build the next one, I'll init the base case, loop through with the transitions formula, and return the final result
# Tricks:
   # If the current state only depends on the last one or two steps, you can ditch the dp array and just use dp variables forward to achieve O(1) space
...
class Solution:
    def maxProfitArray(self, prices: List[int]) -> int:
        ...
        dp = [0] * n
        min_price = prices[0]
        # loop through with the transitions formula
        for i in range(1,n):
            min_price = min(min_price, price[i])  # Update the lowest price seen so far
